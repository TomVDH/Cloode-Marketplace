> **`adjudant` vault?** Clipped notes land as `type: source` (or `type: note`). The shape is the kind's template in `templates/`, parsed into `FIELD_SCHEMA`; see `adjudant:vault-standards (reference/vault-standards.md)` §1. A `source` requires `type:`, `created:`, `updated:`, `verified:` and `verified_by:` (`tested | read | docs`), and may add `source:`. A `note` requires `type:`, `created:` and `updated:`, and may add `session:`. Write no other key: `tags:` and `title:` are fields no template declares and `tidy` strips them. Project membership is the folder path, not a frontmatter field: save the clipped note into the project's folder rather than writing `project:`.
