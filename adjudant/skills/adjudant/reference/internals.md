...
| SessionEnd | `hooks/scripts/sessionend.sh` | Append `session ended` marker only when something was logged since the last hook marker + sync handoff to vault; then reseed an existing board via `board_bridge.py --ensure-only` |
