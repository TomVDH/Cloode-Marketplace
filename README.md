...
|--------|---------|-------------|
| [adjudant](./adjudant) | 0.14.0 | Obsidian vault editor/writer and project initializer. One command, `/adjudant`, with twelve verbs (connect, sync, check, sitrep, tidy, ramasse, dream, draw, board, shelf, advisor, kebab) — each backed by a Python helper. Locked three-tier cleanup model (tidy / ramasse / dream), vault-aware hooks, drift-defense validators. Successor to the retired `obsidian-bridge`. |
...
│   ├── scripts/            # Python helpers (connect, sync, check, sitrep, tidy, …) + validate.py
